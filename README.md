...
A follow-up pass across `tests/tappend.c`, `tdrive.c`, `tctrlz.c`, `tlongfn.c`, `twild.c`, `trenamex.c`, `tsparse.c`, and `tpadread.c` pinned down three more DCCRTL bugs, all since fixed, plus one documented (intentionally unfixed) limitation and one cross-emulator behavioral split that turned out not to be a bug at all:
  ...
  - **Rename onto an existing destination name is a real, cross-emulator behavioral split, not a bug in either camp.** ntvcm, cpmemu, zxcc, z88dk's cpm, and RunCPM all let BDOS 23 (rename) silently overwrite an existing destination name. tnylpo and cpm.exe both reject it instead (`rename()` returns nonzero; the old name and its original content are left untouched). The CP/M 2.2 BDOS spec doesn't clearly mandate either behavior for this case, and DCCRTL's `rename()` doesn't paper over the difference - if your app cares which way this goes, check the return value and/or delete the destination first. See `tests/trenamex.c`, which only asserts the one thing both camps agree on (at least one of the two names survives with valid content) rather than which specific outcome occurs.
