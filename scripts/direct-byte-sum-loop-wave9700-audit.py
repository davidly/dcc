#!/usr/bin/env python3
"""Exhaustively audit the retained direct byte-sum loop schedule."""
...
FUNCTION = "fixture_direct_byte_sum"
RENAMED_FUNCTION = "fixture_direct_byte_sum_renamed"
SOURCE = "tests/mir-clobber/dbytesum.c"
...
BASELINE_SELECTED_HASH = "8aa7f1fe"
...
REJECT_REASON = re.compile(
    ...
    r"template=byte-sum-loop-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "DB93BASE", expect_exact=True),
    SourceControl(
        "renamed", "DB93NAME", ("DBYTESUM_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "unsigned-byte", "DB93UNSG", ("DBYTESUM_UNSIGNED",),
    ),
    SourceControl(
        "volatile-byte", "DB93VOLT", ("DBYTESUM_VOLATILE",),
    ),
    SourceControl(
        "zero-only", "DB93ZERO", ("DBYTESUM_ZERO_ONLY",),
    ),
    SourceControl(
        "add-all", "DB93ADDA", ("DBYTESUM_ADD_ALL",),
    ),
    SourceControl(
        "extra-cfg", "DB93XCFG", ("DBYTESUM_EXTRA_CFG",),
    ),
)
...
def run_runtime_control(
    ...
    runtime_source = build_dir / "DBYTESUM.C"
    ...
    expected_runtime = ("direct byte sum failures=0",)
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/direct-byte-sum-loop-wave9700-audit",
    )
    parser.add_argument(
        "--allow-survivors",
        action="store_true",
    ...
    print(
        f"direct byte sum loop Wave 9700 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
