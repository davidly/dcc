#!/usr/bin/env python3
"""Exhaustively audit the retained memory exercise exact schedule."""
...
CONTROLS = {
    "production": ("tests/tm.c", "main", ()),
    "fixture": ("tests/mir-clobber/mem27.c", "main", ()),
    "renamed": (
        "tests/mir-clobber/mem27.c", "main",
        ("-DMEM27_RENAMED_LOCALS",)
    ),
    "renamed-calls": (
        "tests/mir-clobber/mem27.c", "main",
        ("-DMEM27_RENAMED_CALLS",)
    ),
    "stack-memset": (
        "tests/mir-clobber/mem27.c", "main",
        ("-DMEM27_STACK_MEMSET",)
...
MUTATION_CONTROLS = ("fixture", "stack-memset")
VARIANTS = {
    "no-stack": (),
    "stack": ("-fstack-check",),
...
ABI_MUTATIONS = ("print", "calloc", "check", "memset", "malloc", "free")
...
def exact_accept(output, function):
    return (
        f"function={function} "
        "template=memory-exercise-runner accept=emitted"
    ) in output
...
def baseline_instructions(root, compiler, flags, source, function):
    ...
    if not exact_accept(output, function):
        raise RuntimeError(
            f"memory exercise exact control rejected for "
            f"{source} {flags}\n{output}"
        )
    sections = []
    instructions = None
    ...
    if not sections:
        raise RuntimeError(
            f"expected 386 {function} instructions, got no final MIR"
        )
    return sections[-1]
...
def abi_mutation_survives(
    root, compiler, flags, source, function, role
):
    env = os.environ.copy()
    env.update(
        DCC_MIR_MACHINE_REPORT="1",
        DCC_MIR_SELECT_REPORT="1",
        DCC_MIR_MEMORY_MUTATE_ABI=role,
    )
    output = run(compiler_command(compiler, flags, source), root, env)
    return role if exact_accept(output, function) else None
...
def runtime_controls(root, jobs):
    run(
        [
            ...
            "-Cases",
            "memory-exercise-wave27,memory-exercise-wave27-debug",
            "-Jobs",
            str(jobs),
        ],
        root,
...
def main():
    ...
    for control_name in MUTATION_CONTROLS:
        source, function, control_flags = CONTROLS[control_name]
        for variant_name, flags in variants.items():
            ...
            abi_survivors = [
                role for role in ABI_MUTATIONS
                if abi_mutation_survives(
                    root, compiler, all_flags,
            ...
            print(
                f"{control_name}-{variant_name}: "
                f"{len(jobs) + len(ABI_MUTATIONS)} mutations, "
    ...
    print("memory exercise runner Wave 27 campaign passed")
