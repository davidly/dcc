#!/usr/bin/env python3
"""Exercise every retained constant do-while exact-schedule layout."""
...
LAYOUTS = {
    "production": (
        "tests/tdowhile.c", "test_do_while_behavior", ()
    ),
    "fixture": (
        "tests/mir-clobber/dw5pf.c", "probe_do_while", ()
    ),
    "alternate-counts": (
        "tests/mir-clobber/dw5pf.c", "probe_do_while",
        ("-DDW21_ALT_COUNTS",)
...
def exact_accept(function):
    return (
        f"function={function} "
        "template=constant-do-while-schedule accept=emitted"
    )
...
def baseline_instructions(root, compiler, flags, source, function):
    ...
    if exact_accept(function) not in output:
        raise RuntimeError(
            f"constant do-while exact control rejected for "
            f"{source} {flags}\n{output}"
        )
    instructions = []
    inside = False
    ...
    if len(instructions) != 171:
        raise RuntimeError(
            f"expected 171 {function} instructions, got "
            f"{len(instructions)}"
        )
    return instructions
...
def main():
    ...
    print("constant do-while Wave 21 campaign passed")
