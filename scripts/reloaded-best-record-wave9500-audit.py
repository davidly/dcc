#!/usr/bin/env python3
"""Exhaustively audit the retained reloaded best-record exact schedule."""
...
FUNCTION = "reloaded_best_record_fixture"
RENAMED_FUNCTION = "reloaded_best_record_fixture_renamed"
SOURCE = "tests/mir-clobber/reloadbest.c"
STAGED_SOURCE = "rldbest.c"
...
BASELINE_SELECTED_HASH = "78bade6e"
EXPECTED_RUNTIME = (
    "reloaded best record first=run second=log none=1 checksum=-30716",
)
LOWEST_RUNTIME = (
    "reloaded best record first=log second=log none=1 checksum=-32046",
)
...
REJECT_REASON = re.compile(
    ...
    r"template=best-record-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "RB95BASE", expect_exact=True),
    SourceControl(
        "renamed", "RB95NAME", ("RELOADED_BEST_RECORD_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile-tasks", "RB95VOL",
        ("RELOADED_BEST_RECORD_VOLATILE_TASKS",),
    ),
    SourceControl(
        "unsigned-priority", "RB95UNSG",
        ("RELOADED_BEST_RECORD_UNSIGNED_PRIORITY",),
    ),
    SourceControl(
        "alternate-predicate", "RB95PRED",
        ("RELOADED_BEST_RECORD_ALTERNATE_PREDICATE",),
    ),
    SourceControl(
        "extra-cfg", "RB95CFG", ("RELOADED_BEST_RECORD_EXTRA_CFG",),
    ),
    SourceControl(
        "lowest-priority", "RB95LOW",
        ("RELOADED_BEST_RECORD_LOWEST_PRIORITY",),
        expected_runtime=LOWEST_RUNTIME,
    ),
)
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/reloaded-best-record-wave9500-audit",
    )
    ...
    print(
        f"reloaded best record Wave 9500 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
