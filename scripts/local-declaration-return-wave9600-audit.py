#!/usr/bin/env python3
"""Exhaustively audit the local-declaration-return exact schedule."""
...
FUNCTION = "fixture_local_declaration_return"
RENAMED_FUNCTION = "fixture_local_declaration_return_renamed"
SOURCE = "tests/mir-clobber/localdeclret.c"
...
BASELINE_SELECTED_HASH = "0c83fd27"
...
REJECT_REASON = re.compile(
    ...
    r"template=local-declaration-return-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "LD94BASE", expect_exact=True),
    SourceControl(
        "renamed-function", "LD94NAME", ("LOCALDECLRET_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "renamed-symbols", "LD94SYMS",
        ("LOCALDECLRET_RENAMED_SYMBOLS",), expect_exact=True,
    ),
    SourceControl(
        "changed-return", "LD94RETN",
        ("LOCALDECLRET_CHANGED_RETURN",), expected_result=303,
    ),
    SourceControl(
        "volatile-array", "LD94VOLT",
        ("LOCALDECLRET_VOLATILE_ARRAY",),
    ),
    SourceControl(
        "array-three", "LD94ARRY", ("LOCALDECLRET_ARRAY_THREE",),
    ),
    SourceControl(
        "extra-cfg", "LD94XCFG", ("LOCALDECLRET_EXTRA_CFG",),
    ),
)
...
def run_runtime_control(
    ...
    runtime_source = build_dir / "LOCDECL.C"
    ...
    selector = selection_from(report, control.function)
    if control.expect_exact:
        require_exact(report, control.function, f"{control.name} {mode}")
        outcome = "exact"
    elif selector == "scheduled-machine-cfg" and allow_survivors:
        outcome = "accepted"
    else:
    ...
    expected = (
        "local declaration return failures=0 "
        f"result={control.expected_result} checksum=40"
    )
...
def run_source_controls(
    compiler, dccmake, output_dir, allow_survivors,
):
    rows = []
    for control in SOURCE_CONTROLS:
        for stack_check in (True, False):
            for peep in (True, False):
                rows.append(
                    run_runtime_control(
                        compiler, dccmake, output_dir, control,
                        stack_check, peep, allow_survivors,
                    )
                )
    return rows
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/local-declaration-return-wave9600-audit",
    )
    parser.add_argument(
        "--allow-survivors",
        action="store_true",
        help="report accepted mutations and source controls instead of failing",
    )
    ...
    source_rows = run_source_controls(
        compiler, dccmake, output_dir, args.allow_survivors
    )
    ...
    source_outcomes = Counter(row[2] for row in source_rows)
    print(
        f"local declaration return Wave 9600 "
        f"mutations={len(mutation_rows)} {outcomes}"
    )
    print(
        f"instructions={INSTRUCTION_COUNT} "
        f"fields-per-instruction={len(MUTABLE_FIELDS)}"
    )
    print(f"source controls={len(source_rows)} {source_outcomes}")
