#!/usr/bin/env python3
"""Audit the callback-registration exact runner semantically."""
...
BASELINE_SHA256 = (
    "97144de1185d4dbaf9ec7463c3edb74f19df3ce64790c640e6275bde841f0a70"
)
EXPECTED_OUTCOMES = Counter(
    rejected=87, accepted=307, **{"accepted-changed": 2}
)
EXPECTED_BENIGN = Counter(
    **{"unused-opcode-field": 307, "propagated-string": 2}
)
FIELDS = (
...
def baseline(root, compiler, output):
    ...
    if EXACT_SELECTION.search(report) is None:
        raise RuntimeError(
            "baseline did not select callback-registration-runner"
        )
    instructions = []
    inside = False
    ...
    if [item[0] for item in instructions] != list(range(66)):
        raise RuntimeError(
            f"expected instructions 0..65, got {len(instructions)}"
        )
...
def classify(result):
    ...
    if outcome == "accepted-changed":
        ...
        field == "type"
        and instruction in {19, 25, 28, 30, 34, 37, 43, 46, 48, 52}
    ):
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/callback-registration-wave27-audit",
    )
    ...
    print(
        f"callback registration Wave 27 mutations={len(results)} "
        f"{outcomes}"
    )
