#!/usr/bin/env python3
"""Exhaustively audit the retained best-record exact schedule."""
...
FUNCTION = "best_record_fixture"
RENAMED_FUNCTION = "best_record_fixture_renamed"
SOURCE = "tests/mir-clobber/bestrecord.c"
STAGED_SOURCE = "bstrec.c"
...
EXPECTED_RUNTIME = (
    "best record first=run second=log none=1 checksum=-30716",
)
LOWEST_RUNTIME = (
    "best record first=log second=log none=1 checksum=-32046",
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "BR83BASE", expect_exact=True),
    SourceControl(
        "renamed", "BR83NAME", ("BEST_RECORD_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile-tasks", "BR83VOL", ("BEST_RECORD_VOLATILE_TASKS",),
    ),
    SourceControl(
        "unsigned-priority", "BR83UNSG",
        ("BEST_RECORD_UNSIGNED_PRIORITY",),
    ),
    SourceControl(
        "alternate-predicate", "BR83PRED",
        ("BEST_RECORD_ALTERNATE_PREDICATE",),
    ),
    SourceControl(
        "extra-cfg", "BR83CFG", ("BEST_RECORD_EXTRA_CFG",),
    ),
    SourceControl(
        "lowest-priority", "BR83LOW", ("BEST_RECORD_LOWEST_PRIORITY",),
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/best-record-wave8500-audit",
    )
    ...
    print(
        f"best record Wave 8500 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
