#!/usr/bin/env python3
"""Exhaustively audit the retained initializer-check exact schedule."""
...
FUNCTION = "initializer_check_fixture"
RENAMED_FUNCTION = "initializer_check_fixture_renamed"
SOURCE = "tests/mir-clobber/initcheck.c"
STAGED_SOURCE = "initchk.c"
...
BASELINE_SELECTED_HASH = "6e73ca19"
EXPECTED_RUNTIME = ("initializer check failures=0 checks=6",)
...
REJECT_REASON = re.compile(
    ...
    r"template=initializer-check-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "IC97BASE", expect_exact=True),
    SourceControl(
        "renamed", "IC97NAME", ("INITCHECK_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "changed-value", "IC97VALU", ("INITCHECK_CHANGED_VALUE",),
    ),
    SourceControl(
        "volatile-local", "IC97VOL", ("INITCHECK_VOLATILE",),
    ),
    SourceControl(
        "extra-cfg", "IC97CFG", ("INITCHECK_EXTRA_CFG",),
    ),
)
...
def run_runtime_control(
    ...
    missing = [
        expected for expected in EXPECTED_RUNTIME
...
def run_mutation(compiler, work_dir, case):
    ...
    if outcome == "rejected":
        ...
        if reject != "semantic-payload":
            raise RuntimeError(
                f"{case.name} rejected as {reject!r}, "
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/initializer-check-wave10000-audit",
    )
    ...
    print(
        f"initializer check Wave 10000 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
