#!/usr/bin/env python3
"""Compile-only semantic mutation audit for byte-equality-runner."""
...
EXACT = (
    ...
    "template=byte-equality-runner accept=emitted"
)
GENERIC = re.compile(
...
SHAPES = (("canonical", ROOT / "tests/tbyteeq.c", 380, "5155bee9"),)
MUTATIONS = {
    "type": 32767,
    "memory_size": 32,
    "src1": 999,
...
BENIGN_OPCODES = {
    ...
    "identity": {
        ...
        "binary", "nop", "jump", "phi", "return", "indexaddr",
        "loadind", "storeind",
    },
}
EXPECTED_CLASSIFICATIONS = Counter({
    "rejected": 944,
    "benign-unused-field": 1024,
    "diagnostic-identity": 312,
})
...
def parse_final_instructions(report, count):
    ...
    if list(instructions) != list(range(count)):
        raise RuntimeError(
            f"expected {count} byte-equality instructions, "
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default=str(ROOT / "build/byte-equality-wave28-audit"),
    )
    ...
    print(
        f"byte-equality-wave28: {len(results)} mutations; "
    ...
    if meaningful or counts != EXPECTED_CLASSIFICATIONS:
        if counts != EXPECTED_CLASSIFICATIONS:
            print(
                "byte-equality-wave28: classification drift: "
