#!/usr/bin/env python3
"""Exhaustively audit the retained float-sweep exact schedule."""
...
FUNCTION = "float_sweep_fixture"
RENAMED_FUNCTION = "renamed_float_sweep_fixture"
SOURCE = "tests/mir-clobber/fltsweep.c"
TEMPLATE = "float-sweep-schedule"
FORCED_CANDIDATE = "spilled-phi-slot"
INSTRUCTION_COUNT = 431
BASELINE_SHA256 = (
    "74674d626ad7b36e43e807e1f1f421a3ae5972aacf8acca749a8e1ef38c82827"
)
BASELINE_SELECTED_HASH = "2a78fb3a"
EXPECTED_RUNTIME = "float sweep failures=0 checks=11"
...
REJECT_REASON = re.compile(
    ...
    r"template=float-sweep-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "FS41BASE", expect_exact=True),
    SourceControl(
        "renamed", "FS41NAME", ("FLOAT_SWEEP_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile", "FS41VOL", ("FLOAT_SWEEP_VOLATILE",),
        expected_reject="semantic-payload",
    ),
    SourceControl(
        "extra-cfg", "FS41CFG", ("FLOAT_SWEEP_EXTRA_CFG",),
    ),
)
...
IDENTITY_INSTRUCTIONS = (
    1, 6, 11, 16, 21, 27, 33, 39, 44, 50, 55,
    61, 66, 71, 76, 81, 86, 92, 98, 104,
    110, 115, 120, 125, 130, 135, 140, 145, 151, 156,
    162, 168, 174, 180, 185, 191, 196, 201,
    230, 232, 239, 241, 248, 250, 257, 259,
    292, 294, 301, 303, 346, 348, 393, 398, 408, 417, 428,
)
...
def run_source_control(
    ...
    if not stack_check:
        ...
        if EXPECTED_RUNTIME not in output:
            raise RuntimeError(
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/float-sweep-wave4100-audit",
    )
    ...
    print(
        f"float sweep Wave 4100 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
