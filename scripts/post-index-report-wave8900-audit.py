#!/usr/bin/env python3
"""Exhaustively audit the retained post-index report exact schedule."""
...
FUNCTION = "post_index_report_fixture"
RENAMED_FUNCTION = "post_index_report_fixture_renamed"
SOURCE = "tests/mir-clobber/postindex.c"
STAGED_SOURCE = "postidx.c"
...
BASELINE_SELECTED_HASH = "357dcd2f"
...
REJECT_REASON = re.compile(
    ...
    r"template=post-index-report-schedule reject=(?P<reason>\S+)"
)
...
class SourceControl:
    ...
    report_line: str = (
        "post index global=2 value=10 local=2 value=20"
    )
    checksum_line: str = "post index failures=0 checksum=754"
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "P87BASE", expect_exact=True),
    SourceControl(
        "renamed", "P87NAME", ("POST_INDEX_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "changed-values", "P87VALUE", ("POST_INDEX_CHANGED_VALUES",),
        report_line="post index global=2 value=2345 local=2 value=67",
        checksum_line="post index failures=0 checksum=52171",
    ),
    SourceControl(
        "volatile-array", "P87VOL", ("POST_INDEX_VOLATILE_ARRAY",),
    ),
    SourceControl(
        "unsigned-global", "P87UNSG", ("POST_INDEX_GLOBAL_UNSIGNED",),
    ),
    SourceControl(
        "changed-index", "P87INDEX", ("POST_INDEX_CHANGED_INDEX",),
        report_line="post index global=3 value=10 local=3 value=20",
        checksum_line="post index failures=0 checksum=1011",
    ),
    SourceControl(
        "extra-cfg", "P87CFG", ("POST_INDEX_EXTRA_CFG",),
    ),
)
...
def baseline_compile(compiler, output_dir):
    ...
    report = run(
        ...
        diagnostic_environment(FUNCTION, include_cost=True),
    )
...
def forced_fallback_control(compiler, output_dir):
    ...
    report = run(
        compiler_command(compiler, output_dir / "forced.MAC"),
        environment,
    )
    selector = require_generic(report, FUNCTION, "forced fallback")
    expected = (
        f"MIR cost-selected function={FUNCTION} "
    ...
    if expected not in report:
        raise RuntimeError(
            f"forced fallback selected wrong candidate\n{report}"
        )
    return selector
...
def run_runtime_control(
    ...
    staged_source = build_dir / STAGED_SOURCE
    ...
    for expected in (control.report_line, control.checksum_line):
        if expected not in runtime:
            raise RuntimeError(
                f"{control.name} {mode} missing {expected!r}\n{runtime}"
            )
    return (
        control.name,
...
def run_mutation(compiler, work_dir, case):
    ...
    if selector != "scheduled-machine-cfg":
        selector = require_generic(report, FUNCTION, case.name)
        outcome = "rejected"
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/post-index-report-wave8900-audit",
    )
    ...
    fallback_selector = forced_fallback_control(compiler, output_dir)
    source_rows = []
    ...
    write_tsv(
        ...
        (
            "name", "defines", "selector", "reject_reason", "mode",
            "report_oracle", "checksum_oracle",
        ),
        source_rows,
    )
    write_tsv(
    ...
    print(
        f"post-index report Wave 8900 mutations={len(MUTATION_CASES)} "
        f"{outcomes}"
    )
    ...
    print(f"forced fallback={fallback_selector}")
