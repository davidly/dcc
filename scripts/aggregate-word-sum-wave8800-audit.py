#!/usr/bin/env python3
"""Exhaustively audit the retained aggregate-word-sum exact schedule."""
...
FUNCTION = "aggregate_word_sum_fixture"
RENAMED_FUNCTION = "aggregate_word_sum_fixture_renamed"
SOURCE = "tests/mir-clobber/aggwordsum.c"
RELEASE_SOURCE = "tests/tstructv.c"
STAGED_SOURCE = "aggwsum.c"
TEMPLATE = "aggregate-word-sum-schedule"
FORCED_CANDIDATE = "spilled-phi-slot"
FUNCTIONS = {
    "fixture": (FUNCTION, SOURCE, 23),
    "pair": ("proto_sum_pair", RELEASE_SOURCE, 16),
    "two": ("proto_sum_two", RELEASE_SOURCE, 31),
    "mix": ("proto_sum_mix", RELEASE_SOURCE, 22),
    "wrap": ("proto_sum_wrap", RELEASE_SOURCE, 28),
    "big": ("proto_sum_big", RELEASE_SOURCE, 48),
}
BASELINE_SHA256 = (
    "c12414d78dada56849ccdd1526bafde3190c8ea60c1eb6cfc538760cd6812037"
)
BASELINE_SELECTED_HASH = "238db0ac"
EXPECTED_RUNTIME = (
    "aggregate word sum first=302 second=1079 checksum=6213",
)
SUBTRACT_RUNTIME = (
    "aggregate word sum first=280 second=969 checksum=5729",
)
...
REJECT_REASON = re.compile(
    ...
    r"template=aggregate-word-sum-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "AW88BASE", expect_exact=True),
    SourceControl(
        "renamed", "AW88NAME", ("AGG_WORD_SUM_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile-aggregate", "AW88VOL",
        ("AGG_WORD_SUM_VOLATILE",),
    ),
    SourceControl(
        "unsigned-words", "AW88UNSG",
        ("AGG_WORD_SUM_UNSIGNED",),
    ),
    SourceControl(
        "extra-cfg", "AW88CFG", ("AGG_WORD_SUM_EXTRA_CFG",),
    ),
    SourceControl(
        "subtraction", "AW88SUB", ("AGG_WORD_SUM_SUBTRACT",),
        expected_runtime=SUBTRACT_RUNTIME,
    ),
)
MUTATION_CASES = tuple(
    ...
    for shape, (function, source, instruction_count) in FUNCTIONS.items()
...
def compiler_command(compiler, output, defines=(), source=SOURCE):
    command = [
...
def retained_release_shape_control(compiler, output_dir):
    environment = diagnostic_environment(include_cost=True)
    for name in (
        "DCC_MIR_SELECT_FUNCTION",
    ...
    report = run(
        compiler_command(
            compiler,
            output_dir / "retained-tstructv.MAC",
    ...
    for shape, (function, _, _) in FUNCTIONS.items():
        if shape != "fixture":
            require_exact(report, function, f"retained {shape} shape")
...
def run_mutation(compiler, work_dir, case):
    ...
    report = run(
        compiler_command(
            compiler, output, source=case.source
        ),
        environment,
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/aggregate-word-sum-wave8800-audit",
    )
    ...
    retained_release_shape_control(compiler, output_dir)
    ...
    print(
        f"aggregate word sum Wave 8800 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
        f"instructions={sum(item[2] for item in FUNCTIONS.values())} "
        f"shapes={len(FUNCTIONS)} "
