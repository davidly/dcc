#!/usr/bin/env python3
"""Exhaustively audit the retained unnamed-bitfield report exact schedule."""
...
FUNCTION = "unnamed_bitfield_report"
RENAMED_FUNCTION = "renamed_unnamed_bitfield_report"
SOURCE = "tests/mir-clobber/unbitfld.c"
...
BASELINE_SELECTED_HASH = "c6ee54c1"
EXPECTED_RUNTIME = "unnamed bitfield failures=0"
...
REJECT_REASON = re.compile(
    ...
    r"template=unnamed-bitfield-report reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "UB43BASE", expect_exact=True),
    SourceControl(
        "renamed", "UB43NAME", ("UNBITFIELD_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "extra-cfg", "UB43CFG", ("UNBITFIELD_EXTRA_CFG",),
    ),
    SourceControl(
        "volatile-locals", "UB43VOL", ("UNBITFIELD_VOLATILE",),
    ),
)
...
IDENTITY_INSTRUCTIONS = (
    2, 4, 6, 8, 10, 12, 14, 16,
    19, 23, 30, 33, 37, 44, 47, 54,
    57, 61, 68, 71, 75, 82, 85, 89, 96,
)
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/unnamed-bitfield-report-wave6100-audit",
    )
    ...
    print(
        f"unnamed bitfield report Wave 6100 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
    ...
    if sum(outcomes.values()) != len(MUTATION_CASES):
