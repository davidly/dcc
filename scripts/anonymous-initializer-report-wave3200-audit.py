#!/usr/bin/env python3
"""Audit the anonymous-initializer report exact schedule semantically."""
...
FUNCTION = "anonymous_initializer_report"
RENAMED_FUNCTION = "renamed_anonymous_initializer_report"
SOURCE = "tests/mir-clobber/anoninit.c"
TEMPLATE = "anonymous-initializer-report"
FORCED_CANDIDATE = "spilled-phi-slot"
INSTRUCTION_COUNT = 278
...
REJECT_REASON = re.compile(
    ...
    r"template=anonymous-initializer-report reject=(?P<reason>\S+)"
)
...
BASELINE_SELECTED_HASH = "a893ab33"
EXPECTED_RUNTIME = (
    "version=2.7.15",
    "date=17/6/42",
    "reg=0/3/768",
    ...
    "event=K/4,9",
    "message=OK/404",
    "anonymous init tests passed",
    "anonymous initializer report failures=0",
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "AI27BASE", expect_exact=True),
    SourceControl(
        "renamed", "AI27NAME", ("ANONINIT_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile-failures", "AI27VOL",
        ("ANONINIT_VOLATILE_FAILURES",),
    ),
    SourceControl(
        "variadic-check", "AI27VAR",
        ("ANONINIT_VARIADIC_CHECK",),
    ),
    SourceControl(
        "extra-cfg", "AI27CFG", ("ANONINIT_EXTRA_CFG",),
    ),
)
...
IDENTITY_INSTRUCTIONS = (
    ...
    25, 29, 33, 37, 40, 44, 48, 52, 55, 59, 63, 67,
    70, 74, 77, 81, 85, 89, 92, 96, 100,
    103, 111, 114, 122, 125, 133, 136, 144,
    147, 155, 158, 166, 169, 177, 180, 188,
    191, 201, 204, 212, 215, 223, 226, 234,
    237, 245, 248, 254, 257, 265, 266, 272, 274,
)
...
def run_mutation(compiler, work_dir, case):
    ...
    if selector == "scheduled-machine-cfg":
        ...
        return (
            case.name, case.spec, "survived", selector,
    ...
    return (
        case.name, case.spec, "rejected", selector,
        reject_reason_from(report, FUNCTION) or "",
        FORCED_CANDIDATE,
    )
...
def main():
    ...
    parser.add_argument("--allow-survivors", action="store_true")
    parser.add_argument(
        "--output-dir",
        default="build/anonymous-initializer-report-wave3200-audit",
    )
    ...
    print(
        f"anonymous initializer report Wave 3200 "
    ...
    if outcomes.get("survived", 0) and not args.allow_survivors:
        raise RuntimeError(
            f"{outcomes['survived']} meaningful mutations survived"
        )
    if outcomes.get("rejected", 0) != len(MUTATION_CASES):
        raise RuntimeError(f"unexpected mutation outcomes: {outcomes}")
