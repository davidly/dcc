#!/usr/bin/env python3
"""Audit the recursive-byte minimax exact schedule semantically."""
...
FUNCTION = "MinMax"
SOURCE = "tests/mir-clobber/mmw8.c"
EXACT_ACCEPT = (
    "MIR machine function=MinMax "
    "template=recursive-byte-minimax-schedule accept=emitted"
)
GENERIC_SELECTION = re.compile(
    r"MIR selection function=MinMax "
...
def baseline(root, compiler, output):
    ...
    if EXACT_ACCEPT not in report:
        raise RuntimeError(
            "baseline did not select recursive-byte-minimax-schedule"
        )
    instructions = []
    inside = False
    for line in report.splitlines():
        if line.startswith("; MIR function=MinMax "):
            if instructions:
                break
            inside = True
            continue
        if line.startswith("; MIR summary function=MinMax"):
            break
        if inside:
    ...
    if [item[0] for item in instructions] != list(range(253)):
        raise RuntimeError(
            f"expected instructions 0..252, got {len(instructions)}"
        )
...
def classify(result):
    ...
    if (
        field == "type"
        and instruction in {138, 141, 143, 194, 197, 199}
    ):
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/minimax-wave26-audit",
    )
    ...
    print(f"minimax Wave 26 mutations={len(results)} {outcomes}")
