#!/usr/bin/env python3
"""Exhaustively audit the retained variadic-join-report exact schedule."""
...
FUNCTION = "variadic_join_report"
RENAMED_FUNCTION = "variadic_join_report_renamed"
SOURCE = "tests/mir-clobber/varjoin.c"
...
BASELINE_SELECTED_HASH = "c6508ce3"
EXPECTED_RUNTIME = (
    "variadic join len=25 commas=3 str=alpha, beta, gamma, delta",
    "variadic join failures=0 observed-length=25 expected-length=25",
)
...
REJECT_REASON = re.compile(
    ...
    r"template=variadic-join-report reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "VJ21BASE", expect_exact=True),
    SourceControl(
        "renamed-function", "VJ21RF", ("VARJOIN_RENAMED_FUNCTION",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "renamed-locals", "VJ21RL", ("VARJOIN_RENAMED_LOCALS",),
        expect_exact=True,
    ),
    SourceControl(
        "renamed-helper", "VJ21RH", ("VARJOIN_RENAMED_HELPER",),
        expect_exact=True,
    ),
    SourceControl(
        "volatile-count", "VJ21VC", ("VARJOIN_VOLATILE_COUNT",),
        expected_reject="shape",
    ),
    SourceControl(
        "alternate-flow", "VJ21AF", ("VARJOIN_ALTERNATE_FLOW",),
        expected_reject="shape",
    ),
)
...
IDENTITY_INSTRUCTIONS = (
    1, 15, 16, 18, 20, 21, 23, 24, 25, 26, 27, 31, 32, 39,
    42, 46, 49, 54, 56, 58, 60,
)
...
def require_generic(report, function, context, expected_reject=None):
    ...
    if reject is None:
        raise RuntimeError(
            f"{context} did not report exact-schedule rejection\n{report}"
        )
...
def run_mutation(compiler, work_dir, case):
    ...
    return (
        case.name, case.spec, "rejected", reject,
        selector, FORCED_CANDIDATE,
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/variadic-join-report-wave2300-audit",
    )
    ...
    write_tsv(
        ...
        (
            ...
            "ordinary_selector", "forced_candidate",
        ),
        mutation_rows,
    )
    ...
    print(
        f"variadic join report Wave 2300 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    if outcomes != EXPECTED_MUTATION_OUTCOMES:
        survivors = [
            row[1] for row in mutation_rows if row[2] == "survived"
        ]
        raise RuntimeError(
            f"unexpected mutation outcomes: {outcomes} "
            f"!= {EXPECTED_MUTATION_OUTCOMES}; "
            f"survivors={survivors[:20]}"
        )
