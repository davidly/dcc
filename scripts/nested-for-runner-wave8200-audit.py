#!/usr/bin/env python3
"""Exhaustively audit the retained nested-for runner exact schedule."""
...
FUNCTION = "main"
SOURCE = "tests/mir-clobber/nestfor.c"
...
BASELINE_SELECTED_HASH = "60341a7a"
...
REJECT_REASON = re.compile(
    ...
    r"template=nested-for-call-runner reject=(?P<reason>\S+)"
)
...
def independent_runtime():
    ...
    for prime in range(2, LIMIT + 1):
        if prime * prime <= LIMIT and not composite[prime]:
            for item in range(prime * prime, LIMIT + 1, prime):
                composite[item] = True
    primes = [item for item in range(2, LIMIT + 1)
              if not composite[item]]
    gap = max(right - left for left, right in zip(primes, primes[1:]))
    long_mask = int("".join("1" if item == 2 else "0"
                            for item in range(4)))
    float_mask = int("".join("1" if item == 1 else "0"
                             for item in range(4)))
    pointer_mask = int("".join("1" if item == 3 else "0"
                               for item in range(4)))
    global_mask = int("".join("0" if item == 2 else "1"
                              for item in range(4)))
    stride_sum = sum(range(0, 10, 2))
    down_count = len(range(20, 0, -3))
    return (
        f"nested for primes={len(primes)} gap={gap} "
        f"p79={int(not composite[79])}",
        f"nested index l={long_mask:04d} f={float_mask:04d} "
        f"p={pointer_mask:04d} gp={global_mask:04d} row=000",
        f"nested stride sum={stride_sum} down={down_count}",
    )


LIMIT = 80
EXPECTED_RUNTIME = independent_runtime()
...
def run_runtime_control(compiler, dccmake, output_dir, stack_check, peep):
    name = (
    ...
    runtime = run(
        ["ntvcm", "-p", "-s:0", str(build_dir / "NESTFOR.COM")],
        timeout=45,
    )
    missing = [line for line in EXPECTED_RUNTIME if line not in runtime]
    if missing:
        raise RuntimeError(f"{name} runtime missing {missing!r}\n{runtime}")
    return (
        name, "runtime", "-", "exact",
        selection_from(report) or "", "", name,
    )
...
def run_mutation(compiler, work_dir, case):
    ...
    return (
        ...
        selector or "", reject_reason_from(report) or "",
        "ordinary-selection",
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/nested-for-runner-wave8200-audit",
    )
    ...
    print(
        f"nested-for runner Wave 8200 mutations={len(MUTATION_CASES)} "
        f"{outcomes}"
    )
    print(
    ...
    print(f"runtime oracle={' | '.join(EXPECTED_RUNTIME)}")
