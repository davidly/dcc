#!/usr/bin/env python3
"""Exhaustively audit the retained raw-conversion-check exact schedule."""
...
FUNCTION = "raw_conversion_check_fixture"
RENAMED_FUNCTION = "renamed_raw_conversion_check_fixture"
SOURCE = "tests/mir-clobber/rawconv.c"
...
BASELINE_SELECTED_HASH = "e18d2405"
EXPECTED_RUNTIME = (
    "checks=40 failures=0",
    "RESULT: PASS",
    "raw conversion oracle failures=0",
)
...
REJECT_REASON = re.compile(
    ...
    r"template=raw-conversion-check-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "RC70BASE", expect_exact=True),
    SourceControl(
        "renamed", "RC70NAME", ("RAWCONV_RENAMED",),
...
IDENTITY_INSTRUCTIONS = tuple(range(INSTRUCTION_COUNT))
...
def compiler_command(compiler, output):
    return [
        str(compiler), "-fstack-check", "-stack", "512",
        "-ffloatio", "-flongio", "-I", ".", SOURCE, "-o", str(output),
    ]
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/raw-conversion-check-wave8000-audit",
    )
    ...
    print(
        f"raw conversion check Wave 8000 "
        f"mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
