#!/usr/bin/env python3
"""Exhaustively audit the retained for-initializer sum schedule."""
...
FUNCTION = "fixture_for_init_sum"
RENAMED_FUNCTION = "fixture_for_init_sum_renamed"
SOURCE = "tests/mir-clobber/forinitsum.c"
...
BASELINE_SELECTED_HASH = "adb96508"
EXPECTED_RUNTIME = ("for init sum failures=0 checksum=715",)
...
REJECT_REASON = re.compile(
    ...
    r"template=for-init-sum-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "FI86BASE", expect_exact=True),
    SourceControl(
        "renamed", "FI86NAME", ("FORINITSUM_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "unsigned-parameter", "FI86UNSG",
        ("FORINITSUM_UNSIGNED_PARAMETER",),
    ),
    SourceControl(
        "volatile-sum", "FI86VOLT", ("FORINITSUM_VOLATILE_SUM",),
    ),
    SourceControl("long-sum", "FI86LONG", ("FORINITSUM_LONG_SUM",)),
    SourceControl(
        "canonical-loop", "FI86ZERO", ("FORINITSUM_ZERO_START",),
    ),
    SourceControl("extra-cfg", "FI86XCFG", ("FORINITSUM_EXTRA_CFG",)),
)
...
def run_runtime_control(
    ...
    runtime_source = build_dir / "FINIT.C"
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/for-init-sum-wave8700-audit",
    )
    parser.add_argument(
        "--allow-survivors",
        action="store_true",
    ...
    print(
        f"for-init sum Wave 8700 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
