#!/usr/bin/env python3
"""Exhaustively audit the retained arrow-path exact schedule."""
...
FUNCTION = "arrow_path_fixture"
RENAMED_FUNCTION = "arrow_path_fixture_renamed"
SOURCE = "tests/mir-clobber/arrpath.c"
...
BASELINE_SELECTED_HASH = "f6139204"
EXPECTED_RUNTIME = ("arrow path failures=0",)
...
REJECT_REASON = re.compile(
    ...
    r"template=arrow-path-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "AP61BASE", expect_exact=True),
    SourceControl(
        "renamed-function", "AP61NAME", ("ARROWPATH_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile-path", "AP61VOL", ("ARROWPATH_VOLATILE_PATH",),
    ),
    SourceControl(
        "extra-cfg", "AP61CFG", ("ARROWPATH_EXTRA_CFG",),
    ),
)
...
IDENTITY_INSTRUCTIONS = (
    1, 2, 3, 4, 15, 31, 42, 47, 54, 63, 64, 70, 79, 80, 82, 88,
    101, 103, 111, 112, 113, 121, 122, 127, 129,
)
MUTATION_CASES = (
    tuple(
        MutationCase(
