#!/usr/bin/env python3
"""Exhaustively audit the retained pointer-cast-diff schedule."""
...
FUNCTION = "fixture_pointer_cast_diff"
RENAMED_FUNCTION = "fixture_pointer_cast_diff_renamed"
SOURCE = "tests/mir-clobber/ptrcastdiff.c"
...
BASELINE_SELECTED_HASH = "d4f39cb6"
...
REJECT_REASON = re.compile(
    ...
    r"template=pointer-cast-diff-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "PC89BASE", expect_exact=True),
    SourceControl(
        "renamed", "PC89NAME", ("PTRCASTDIFF_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "changed-count", "PC89CNT", ("PTRCASTDIFF_CHANGED_COUNT",),
        expected_checksum=3996,
    ),
    SourceControl(
        "volatile-array", "PC89VOLT",
        ("PTRCASTDIFF_VOLATILE_ARRAY",),
    ),
    SourceControl(
        "unsigned-diff", "PC89UNSG",
        ("PTRCASTDIFF_UNSIGNED_DIFF",),
    ),
    SourceControl(
        "alternate-failure", "PC89FAIL",
        ("PTRCASTDIFF_ALT_FAILURE",),
    ),
    SourceControl(
        "extra-cfg", "PC89XCFG", ("PTRCASTDIFF_EXTRA_CFG",),
    ),
)
...
def run_runtime_control(
    ...
    runtime_source = build_dir / "PTRDIFF.C"
    ...
    expected_runtime = (
        "all pointer-cast diffs correct",
        "pointer cast diff failures=0 "
        f"checksum={control.expected_checksum}",
    )
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/pointer-cast-diff-wave9100-audit",
    )
    parser.add_argument(
        "--allow-survivors",
        action="store_true",
    ...
    print(
        f"pointer cast diff Wave 9100 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
