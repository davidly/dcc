#!/usr/bin/env python3
"""Exhaustively audit the retained board-attack exact schedule."""
...
FUNCTION = "board_attack_fixture"
RENAMED_FUNCTION = "board_attack_fixture_renamed"
SOURCE = "tests/mir-clobber/bdattack.c"
TEMPLATE = "board-attack-schedule"
FORCED_CANDIDATE = "spilled-phi-slot"
INSTRUCTION_COUNT = 676
BASELINE_SHA256 = (
    "fae46f9c6ebbd1d2cc1dc6af6b716c607e85cc1d06b502bb7f61c3a104fde630"
)
BASELINE_SELECTED_HASH = "7e3a2471"
EXPECTED_RUNTIME = (
    "board attack failures=0 checksum=3027",
)
...
REJECT_REASON = re.compile(
    ...
    r"template=board-attack-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "BA25BASE", expect_exact=True),
    SourceControl(
        "renamed-function", "BA25NAME", ("BOARDATTACK_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile-board", "BA25VBRD", ("BOARDATTACK_VOLATILE_BOARD",),
    ),
    SourceControl(
        "volatile-directions", "BA25VDIR",
        ("BOARDATTACK_VOLATILE_DIRECTIONS",),
    ),
    SourceControl(
        "extra-cfg", "BA25CFG", ("BOARDATTACK_EXTRA_CFG",),
    ),
)
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/board-attack-wave6000-audit",
    )
    ...
    print(
        f"board attack Wave 6000 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
