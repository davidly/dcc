#!/usr/bin/env python3
"""Exhaustively audit the retained union-alias runner exact schedule."""
...
FUNCTION = "union_alias_fixture"
RENAMED_FUNCTION = "union_alias_fixture_renamed"
SOURCE = "tests/mir-clobber/unionalias.c"
...
BASELINE_SELECTED_HASH = "47bc9ce9"
EXPECTED_RUNTIME = (
    "union alias fixture passed",
    "union alias failures=0",
)
...
REJECT_REASON = re.compile(
    ...
    r"template=union-alias-runner-schedule reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "UA84BASE", expect_exact=True),
    SourceControl(
        "renamed-function", "UA84NAME", ("UNION_ALIAS_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "volatile-word", "UA84VOL", ("UNION_ALIAS_VOLATILE_WORD",),
    ),
    SourceControl(
        "extra-cfg", "UA84CFG", ("UNION_ALIAS_EXTRA_CFG",),
    ),
    SourceControl(
        "alternate-values", "UA84ALT", ("UNION_ALIAS_ALT_VALUES",),
    ),
)
...
def run_runtime_control(
    ...
    runtime_source = build_dir / "ua84.c"
    shutil.copy2(ROOT / SOURCE, runtime_source)
    command = [
        str(dccmake),
        f"dcc-input={runtime_source}",
...
def run_mutation(compiler, work_dir, case):
    ...
    if selector is None:
        raise RuntimeError(
            f"{case.name} did not report a selected candidate\n{report}"
        )
    outcome = (
        "accepted"
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/union-alias-runner-wave8400-audit",
    )
    ...
    print(
        f"union alias runner Wave 8400 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
