#!/usr/bin/env python3
"""Exhaustively audit the retained pi-digit exact schedule."""
...
FUNCTION = "pi_digit_fixture"
RENAMED_FUNCTION = "renamed_pi_digit_fixture"
SOURCE = "tests/mir-clobber/pidigit.c"
TEMPLATE = "pi-digit"
...
BASELINE_SELECTED_HASH = "050cebd0"
EXPECTED_RUNTIME = "pi digit failures=0 digits=243f6a88"
...
REJECT_REASON = re.compile(
    ...
    r"template=pi-digit reject=(?P<reason>\S+)"
)
...
SOURCE_CONTROLS = (
    SourceControl("baseline", "PD40BASE", expect_exact=True),
    SourceControl(
        "renamed", "PD40NAME", ("PIDIGIT_RENAMED",),
        RENAMED_FUNCTION, expect_exact=True,
    ),
    SourceControl(
        "alternate-bound", "PD40BND", ("PIDIGIT_ALTERNATE_BOUND",),
    ),
)
...
IDENTITY_INSTRUCTIONS = (8, 16, 24, 31, 36, 69)
MUTATION_CASES = tuple(
    ...
    MutationCase("assert-string-67", "67:immediate:0"),
)
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="build/pi-digit-wave4000-audit",
    )
    ...
    print(
        f"pi digit Wave 4000 mutations={len(mutation_rows)} "
        f"{outcomes}"
    )
    print(
