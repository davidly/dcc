...
## Item T56: fix the shared "materialize 0/1 boolean" primitive to use a one-label skip-on-false shape (2026-08-02)

**Hypothesis** (root-caused during Items T54/T55, see their entries
above): every site that turns a Z80 flag test into an explicit 0/1 in
HL used a two-label shape:

```
        <flags already set by a compare/or>
        ld hl,0
        jp <true-cond>,Ltrue
        jp Lend
Ltrue:  inc l                  ; or inc hl
Lend:
```

This always executes `ld hl,0` (10 T-states) on *both* paths, then on
the false path pays an extra unconditional `jp Lend` (10 T-states) that
does nothing but skip an instruction it could instead have branched
around directly. Branching to `Lend` on the *inverted* condition
instead - skipping the increment outright - needs only one label and
drops the unconditional jump entirely:
...
This is 3 bytes/10 T-states smaller on every false-path execution, with
the true path completely unchanged (still `ld hl,0` + `inc l`/`inc hl`).
This exact one-label shape was already used correctly at three sites in
`dcc_mir_spilled_cfg.c` (bool-typed byte loads via `MIR_LOAD_INDIRECT`
and the ix-relative-offset memory-load path) - the fix is simply making
the remaining sites consistent with it, not inventing a new shape.

Four sites still used the older, wasteful two-label shape:

1. `mir_emit_scalar_compare` (`dcc_mir_emit_common.c`) - the general
   scalar `==`/`!=`/`<`/`>`/`<=`/`>=` comparison-to-bool materializer,
   used by every accepted function containing an explicit boolean
   comparison result (by far the highest-traffic site).
2. `mir_emit_scalar_compare_biased_right` (`dcc_mir_emit_common.c`) -
   the sign-pre-biased-right-operand variant of the same thing.
3. `mir_emit_cast`'s bool-cast case (`dcc_mir_spilled_cfg.c`) - "cast to
   `_Bool`" (any-nonzero-value-to-0/1) conversion.
4. The `MIR_UNARY '!'` case (`dcc_mir_spilled_cfg.c`) - logical-not.

Note: `mir_emit_wide_operation`'s own `==`/`!=` case (`dcc_mir_spilled_
cfg.c` ~line 3103) already used the *other* correct shape (branch to a
true-label, `ld hl,0`/`jp end`, `true: ld hl,1`/`end:` - matching
`dcc_cmp.c`'s legacy `emit_cmp_subtract_to_bool` exactly, 12 bytes) and
needed no change; it doesn't use the `ld hl,0`-then-`inc` trick at all
since the wide path already needs a full `ld hl,1`, not an increment.

**Fix**: converted all four sites (dropping the extra label and
unconditional jump in each, inverting the branch condition sense to
target the shared `end_label` directly). No new mechanism, no new
predicate - purely reshaping existing emission strings.

**Validation** (this item touches every already-accepted function that
materializes an explicit boolean, so - per this item's own flagged risk
in the plan above - the full existing MIR-accepted corpus was validated,
not just newly-affected functions):
- Whole-corpus census (`--fail-on-regression`) vs. a clean pre-T56
  baseline (467/2023, 23.08%): **0 regressions** (467/467 MIR-accepted
  functions stayed MIR-accepted; none flipped to fallback). 109 apps
  showed census metric changes (smaller MIR-side generated-byte
  estimates almost everywhere a boolean is materialized, including many
  still-fallback functions whose hypothetical MIR size shrank without
  changing their fallback outcome).
- One anomaly investigated and cleared: `tctxflt.cond_cmparm` appeared
  in the "after" census for the first time (it had no MIR selection
  report line at all in "before"). Traced this to `new_label()` being a
  single global counter shared by MIR emission and the *legacy* AST
  backend (`dcc_cmp.c` and `try_speculative_noix_function_body`,
  `dcc_func.c`) - fewer labels allocated earlier in the same
  translation unit shifted `try_speculative_noix_function_body`'s
  buffered-body decision for this one function from "safe to omit the
  IX frame" to "needs the IX frame" (visible as `ld hl,2/add hl,sp`
  sp-relative addressing before vs. `push ix/ld ix,0/add ix,sp` after).
  Both before and after remain legacy-replayed fallback output (this
  function's own MIR analysis reaches `result=fallback reason=
  instruction-count` in "after"), so this is a missed micro-
  optimization opportunity for one function's frame layout, not a
  correctness regression - confirmed deterministic (re-ran the
  pre-change binary twice, byte-identical output both times) and
  confirmed harmless by the full-mode run below (`tctxflt` passed with
  0 regressions and only improvements).
- Focused `runall.ps1 -Mode full` on all 14 apps the census flagged as
  requiring runtime validation (`t2denum,tanonagg,tautolcs,tbug2,
  tctxflt,tenumfsm,thoistbc,tinline,tlocalfp,tlongopt,tmirfuse,tsetjmp,
  tstr2,wumpus`): **14/14 passed**, 0 regressions, 5 improvements
  (`tanonagg`/`tbug2`/`tenumfsm`/`tautolcs`/`tlocalfp`, all peep cycles).
- Wide safety net, full 323-app corpus, both `-Mode fast` and `-Mode
  full` (peep+nopeep): **314/314 passed** each run, 0 failures.
  `-Mode full`'s performance check: **0 regressions, 167 improvements**
  spread across the corpus (cycle-count and/or byte-size reductions in
  peep and/or nopeep for dozens of apps having nothing to do with the
  `okb`/`chki` family this item was originally motivated by - confirming
  the primitive is exactly as high-traffic as expected).

**Disposition**: landed. This closes out the root cause identified
during Items T54/T55: the shared boolean-materialization primitive
itself is now as cheap as legacy's equivalent on the common (false)
path, with zero blast radius found in the entire corpus. Item T54 (the
`okb`/`chki` stack-forwarding relaxation) can now be safely re-attempted
per the plan's dependency note, since its regression was traced to this
exact primitive, not the relaxation itself.

**Temp files used and cleaned up**: `build/mir-t56-before.tsv`,
`build/mir-t56-after.tsv`, `build/mir-t56-tctxflt-recheck.tsv`,
`/tmp/tctxflt_before*.mac`, `/tmp/tctxflt_after.mac`, `/tmp/before_
stderr*.txt`, `/tmp/after_stderr.txt` (all census/mac scratch files,
not committed per policy).
