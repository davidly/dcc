...
## Item T386: phi-fallthrough-cost threshold, -10 to -9 (2026-08-14)

The `phi-fallthrough-cost` gate (`dcc_mir_select.c`) declined any
`spilled-scalar-cfg` candidate whose generated-instruction margin was less
than 10 instructions below captured, per its own comment: "the complete
measured population below a ten-instruction win regressed at least one
runtime mode." `forint:ensure_sym` sat exactly one instruction short of that
bar (`generated=43, captured=52`, delta -9) - the single closest
`phi-fallthrough-cost` near-miss in the current census, previously assessed
(T384's segment) as needing a larger architectural fix (generalizing HL
forwarding across labels/jumps for multi-predecessor phi joins).

Re-examining its actual MIR (`DCC_MIR_REPORT=1`) shows a genuine two-
predecessor phi join at `L2` (`v13 = phi(v2 from L3, v12 from L4)`), not the
simple adjacent-forwarding-restriction case originally hypothesized - it is
a real, already-correct multi-predecessor merge that is 9 instructions
smaller than the legacy fallback, just one short of the existing blanket
-10 cutoff. Since this is the *only* function anywhere in the census that
newly qualifies if the cutoff moves to -9 (`too:bst_height`, the next
nearest, is at -5), this was a cheap, fully bounded, single-function
experiment: loosen the threshold by one instruction, rebuild, and measure.

Focused validation (`runall.ps1 -Mode full -Apps forint`) and the mandatory
full extended gate (`-Mode full -Extended -RunTimeout 30`) both pass with
zero regressions in either runtime mode; `forint`'s existing large cycle win
(peep -31.38%, from earlier cohorts) is unchanged, confirming `ensure_sym`
itself is not a hot path in that benchmark but the admission is free -
strictly smaller code with no measured downside. Fresh census: ordinary
**891/2026 (43.98%, +1)**, stack-check **913/2128 (42.90%, +1)**, zero
regressions/removals in either mode.

This does not contradict the original -10 threshold's own comment - it
narrows the false-negative by exactly the one instruction margin that
separates a real, measured win from the previously-tested regressing
population, without reopening any of the actually-regressing candidates
(none of which are within reach of -9). The bigger phi-forwarding-across-
labels architectural lead identified in T384's segment remains open and
unimplemented; this item resolves the specific `ensure_sym` case without it.
