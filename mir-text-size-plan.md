...
## Item T411: existing address-rematerialization retry also clears half of `planned-index-base-cost`'s current population, but not enough to promote anything (+0, 2026-08-07)

Follow-up on the current `planned-index-base-cost` round began by re-reading
the actual retry path in `dcc_mir_select.c` before inventing any new
mechanism. Key finding: the existing address-rematerialization retry already
has the exact structural behavior this bucket's simplest address-rooted
helpers need. When `mir_address_rematerialization_enabled` is active,
`mir_prepare_backend_slots()` skips slot/handoff planning entirely for a
single-use `MIR_ADDRESS` candidate before it ever reaches
`mir_planned_stack_consumer()`, so the later `MIR_INDEX_ADDRESS` reloads the
base directly from `mir_emit_virtual_load()` instead of paying the
planned-stack push/pop pair. In other words, the needed emitter work was
already implemented; production simply only retried it for
`instruction-count`/`text-size`, never for `planned-index-base-cost`.

**Change landed in this branch:** extend that one existing retry trigger to
also fire when the provisional fallback reason is `planned-index-base-cost`
(`dcc_mir_select.c`). No new selector, no new rematerialization rule, no new
gate threshold - just let the already-proven address-rematerialization retry
re-run the same spilled selector for this bucket too.

**What actually changed, from fresh baseline census comparisons:**

- Ordinary: `planned-index-base-cost` **38 -> 19** functions.
- Stack-check: `planned-index-base-cost` **45 -> 26** functions.
- Coverage: **unchanged** at **908/2026 ordinary (44.82%)** and
  **930/2128 stack-check (43.70%)**.
- Newly MIR-emitted: **0** in both modes.
- Already-accepted MIR output changes: **0** (`apps requiring runtime
  validation: 0` in both compares).

The retry is therefore a **candidate-stream-quality-only enabler** with zero
blast radius on currently shipped binaries, exactly like T402's coverage-neutral
direct-reload storeind work.

**Ordinary changed-function breakdown (24 total):**

1. **Address-rooted helpers where the planned-index-base dependency vanished
   entirely and the function now falls through to `text-size` instead:**
   `cobint.{vpush,vpop}`, `tinline.{nest_give,nest_take,vpush2,vpop2,
   edge_outer,mem_get,mem_set}`, `tinlinfb.{guard_push,guard_pop}`,
   `tinlnpar.mem_store`, `tctxops.ca_index`, `too.tnode_new`, and
   `fileops.main`.
2. **Real static shrink, but still blocked by another measured gate:**
   `attnc11.{make_targets,project_logits}`, `too.{machine_run,node_new}`,
   `trowptr.scalar_target`, `tvolopt.volatile_anonymous_member_reload`,
   `adaint.var_or_const_decl`, `cobint.emit`, and `tenumfsm.scan`.
3. **Mixed caution:** `fileops.main` is the one current ordinary loser
   (7037 -> 7322 generated bytes) even though it also leaves
   `planned-index-base-cost`; this is why the change is being recorded as a
   reusable retry/enabler, not claimed as a generally profitable size win.

Stack-check shows the same pattern plus two additional address-heavy helpers
that improve only under stack-check instrumentation:
`tfarrsub.{set_grid,set_param}`.

**Important correctness note:** this change does **not** solve the existing
forced-accept correctness bugs already recorded for
`adaint.var_or_const_decl` and `tenumfsm.scan`; it only changes which later
cost gate rejects them after the address-rooted planned-stack handoff is
removed. Their `mir-dead-ends.tsv` entries remain binding.

**Interpretation:** the current bucket's address-rooted slice was never asking
for a brand-new direct-reload/index-base mechanism after all. The production
path already knew how to rematerialize those bases; it just was not wired into
this fallback reason. Wiring it in removes that handoff from about half the
current population, but most of those functions immediately expose a deeper
limiter (`text-size`, `rhs-stack-cost`, `branch-condition-cost`,
`indirect-store-address-cost`, or a still-unimproved planned-index case).
So this is **real infrastructure**, but not a complete solution for the
bucket by itself.

**Validation:**

- `sh src/dcc/build-dcc.sh`
- Ordinary census compare vs `build/mir-before.tsv`: zero coverage
  regressions, 24 changed fallback rows, 0 runtime-validation apps.
- Stack-check census compare vs `build/mir-before-stackcheck.tsv`: zero
  coverage regressions, 26 changed fallback rows, 0 runtime-validation apps.
- Full extended gate after rebuilding the whole host toolchain with
  `pwsh ./scripts/build-dcc.ps1`:
  **314/323 passed, 0 failed, 9 skipped; diagnostics/dccpeep/extended/perf all
  passed** (~36.2s).

**Disposition:** worth keeping as a coverage-neutral reusable enabler. It
materially shrinks 9 ordinary / 11 stack-check still-fallback candidates,
...
## Item T414: `absolute-index-cost` already has the selector; the safe win this wave is only a coverage-neutral rematerialization retry (+0, 2026-08-08)

Fresh baseline on integrated `3fe63a8`:
**908/2026 ordinary (44.82%)**, **930/2128 stack-check (43.70%)**.
`absolute-index-cost` was still **30 ordinary / 31 stack-check**.

I started by re-reading the exact gate and the existing emitter plumbing before
changing anything. The gate is a pure 4% instruction-margin check in
`dcc_mir_select.c` that fires only when the spilled CFG selector already
reports `mir_spilled_cfg_depends_on_constant_index_absolute()`. And the actual
constant-absolute indexed machinery is already present: the centralized
resolver in `dcc_mir_emit_common.c` folds `ADDRESS`/`MEMBER_ADDRESS` plus
constant `INDEX_ADDRESS`, while `dcc_mir_spilled_cfg.c` already emits the
resulting constant absolute indexed loads, stores, and address values.
So this bucket is **not** "missing absolute indexed addressing support" -
it is "functions already using that support, but still failing profitability
or another later gate."

The one real structural wiring gap I could still find was the same existing
address-rematerialization retry from T411. That retry already removes the
single-use `MIR_ADDRESS` slot/handoff before an `MIR_INDEX_ADDRESS` consumer,
but production only re-ran it for `instruction-count`, `text-size`, and
`planned-index-base-cost`, not for `absolute-index-cost`.

**Bounded probe first:** extending the retry to all `absolute-index-cost`
misses cut the bucket to **21 ordinary / 22 stack-check** and admitted exactly
one new function: `tc89init.main`. Focused full-mode validation showed why
that broad form is not safe to keep:
`tc89init` remained correctness-clean but regressed peep cycles
**36096 -> 36130 (+0.09%)** even though nopeep improved.

So the production change kept the same retry, but only for
`absolute-index-cost` candidates whose **pre-retry generated bytes are not
already smaller than legacy**. That leaves the known raw-byte-smaller
near-misses (`tc89init.main` and the existing `tptrlhs.touch_ptr_to_array_deref`
correctness bug) on the old guard, while still letting the retry clean up the
larger positive-byte residue.

**Change landed in this branch:** extend the existing address-rematerialization
retry trigger to `absolute-index-cost`, guarded so it only runs for
non-byte-smaller provisional candidates (`dcc_mir_select.c`). No new selector,
no new addressing mode, no threshold adjustment - just reuse the already-proven
retry where it is structurally relevant and currently safe.

**What actually changed, from fresh baseline census comparisons:**

- Ordinary: `absolute-index-cost` **30 -> 23** functions.
- Stack-check: `absolute-index-cost` **31 -> 24** functions.
- Coverage: **unchanged** at **908/2026 ordinary (44.82%)** and
  **930/2128 stack-check (43.70%)**.
- Newly MIR-emitted: **0** in both modes.
- Already-accepted MIR output changes: **0** (`apps requiring runtime
...
**Ordinary changed-function breakdown (22 rows, all sourced from the old
`absolute-index-cost` bucket):**

1. **Still `absolute-index-cost`, but materially smaller after the retry removes
   the address-rooted handoff:** `attnc11.{backward_pass,forward_attention}`,
   `fileops.read_and_validate`, `mm.fmatmult`, `t2darr.main`, `tarray6.main`,
   `tctxflt.main`, `tfpcall.main`, `tlongidx.main`, `tlongsub.main`,
   `tnegidx.main`, `tnestfor.main`, `tptr2dv.main`, `treg.test_scan`, and
   `tsyntax.test_additive_subscripts`.
2. **Now clearly blocked by another existing mechanism instead of
   `absolute-index-cost`:**
   - `wide-store-cost`: `tc89flta.main`, `tc89fltc.main`,
     `too.test_size_inference2`
   - `block-cse-cost`: `tfarrsub.main`, `tfloat4.test_arrays`
   - `branch-condition-cost`: `tchess.ch_computer_move`
   - `cfg-backedge`: `tsyntax.test_constexpr_static_init_and_bounds`

Stack-check shows the same pattern, plus `tchess.undo_move` shrinking while
remaining in `absolute-index-cost`, and `tfarrsub.main` exposing
`planned-index-base-cost` instead of `block-cse-cost`.

**Interpretation:** the current `absolute-index-cost` bucket is not waiting on
another missing absolute/static indexed emitter feature. The production path
already knows how to form and emit those addresses, and it already had the
right rematerialization machinery too. Wiring that retry into this fallback
reason removes the address-rooted handoff from about a quarter of the current
population, but most of those functions immediately expose a deeper blocker
(`wide-store-cost`, `block-cse-cost`, `branch-condition-cost`,
`cfg-backedge`, or a still-large residual `absolute-index-cost` miss).

So the safe result this round is a **coverage-neutral reusable enabler**:
it sharpens the bucket to the harder residue without changing any shipped
binary, and it confirms that future work here should rank the newly exposed
later gates rather than assuming `absolute-index-cost` still lacks core
addressing support.
...
- `sh src/dcc/build-dcc.sh`
- Ordinary census compare vs `build/mir-before.tsv`: zero coverage
  regressions, 22 changed fallback rows, 0 runtime-validation apps.
- Stack-check census compare vs `build/mir-before-stackcheck.tsv`: zero
  coverage regressions, 23 changed fallback rows, 0 runtime-validation apps.
- Focused bounded probe on the unguarded form:
  `pwsh ./scripts/runall.ps1 -Apps tc89init -Mode full -RunTimeout 20`
  -> correctness-clean, but peep regression **36096 -> 36130 (+0.09%)**;
  not kept.
- Full extended gate after rebuilding the whole host toolchain with
  `pwsh ./scripts/build-dcc.ps1`:
...
## Item T425: recover the `tbool.set_bool` single-use pointer win with a cheaper direct-home path, without re-admitting T424's `+1/-3` object-promotion regression (+1, 2026-08-08)

T424 already proved the semantic side: the per-reference pointer-parameter
classifier is real and distinguishes safe dereference/return uses from opaque
call forwarding. The unresolved question was **why** the broad "keep every
classifier-safe single-use pointer parameter as an object" experiment still
regressed three tiny safe shapes (`tc99apar.read_paren_const`,
`tc99apar.read_paren_restrict`, `tdecl.pick_same_node`) while helping one
(`tbool.set_bool`), and whether a **narrower** path could keep only the real
win.

### 1. Instruction-level root cause

I re-dumped the four exact functions and compared the current production path
against the broadened-object path.

#### Current production path (existing profitability sub-filter intact)

`tc99apar.read_paren_const` / `read_paren_restrict` are already on a **0-slot**
MIR path:
...
Measured current selection:

- `read_paren_const`: **139 generated bytes / 12 insns**, `captured=184/19`
- `read_paren_restrict`: **139 / 12**, `captured=184/19`

`tdecl.pick_same_node` is also already minimal on the current path:
...
- `pick_same_node`: **101 generated bytes / 8 insns**, `captured=91/9`

So the three "losers" already **do not have the redundant slot/home problem at
all** on the current non-object path. They are already zero-slot direct
parameter reloads.

`tbool.set_bool` was different. Before this item, its current non-object MIR
path manufactured a full backend-slot round trip for the pointer address:
...
Measured current forced-MIR selection before the fix:

- `set_bool`: **338 generated bytes / 31 insns**, `captured=187/19`

That is the real structural asymmetry:

- the three T424 losers already had **no pointer slot to remove**
- `set_bool` still had exactly the redundant "spill pointer parameter to a slot,
  reload later as the store address" dance T424 suspected

#### What the broadened object-promotion path was doing

Using a scratch reproduction of the four shapes plus
`DCC_MIR_EMIT_FUNCTION`/`DCC_MIR_REPORT` with the T424 widening re-applied:

```sh
PATH=$PWD:/home/dave/GitHub/ntvcm:$PATH \
DCC_MIR_EMIT_FUNCTION=read_paren_const \
DCC_MIR_REPORT=1 DCC_MIR_SELECT_REPORT=1 \
./dcc -stack 512 -I . build/t425shp.c -o build/T425WIDE-EMIT.MAC
```

the widened MIR summaries showed:

- `read_paren_const`: `objects=1 promoted-loads=1`, max-live 1, spills 0
- `read_paren_restrict`: `objects=1 promoted-loads=1`, max-live 1, spills 0
- `pick_same_node`: `objects=2 promoted-loads=2`, max-live 2, spills 0
- `set_bool`: `objects=2 promoted-loads=2`, max-live 2, spills 0

So the full object-promotion path was indeed the only semantic change. The
important finding is that for the three loser shapes it buys **nothing useful**:
their current path already has zero backend slots and only one direct use, so
there is no repeated load/store traffic for object promotion to amortize. The
full object path only perturbs selector/arbitration cost, while `set_bool`
really does have one removable slot dance.

### 2. Narrower cheaper path

Instead of widening `mir_filter_pointer_parameter_objects()` again, I fixed the
actual cost source directly in `src/dcc/dcc_mir_spilled_cfg.c`.

New helper:

- `mir_pointer_value_has_single_safe_named_home_use()`

It recognizes the exact narrow class that `set_bool` needs:

- definition is a pointer `MIR_PARAM`/`MIR_LOAD`
- objectless incoming `SC_PARAM` named home remains stable
- exactly **one** real use
- that use is one direct dereference / address formation / return
  (`MIR_LOAD_INDIRECT`, `MIR_INDEX_LOAD`, `MIR_INDEX_ADDRESS`,
  `MIR_MEMBER_ADDRESS`, `MIR_STORE_INDIRECT`, or `MIR_RETURN`)
- any `MIR_ARG`/call-forwarding use still rejects immediately

Then `mir_value_has_direct_named_home()` now allows this narrow class even when
the pointer parameter is **not** an MIR object. That means the spilled backend
can skip creating a backend slot and simply reload the original incoming
`ix+N` parameter address at the one use site.

This is intentionally cheaper than full object promotion:

- no object admission/filter change
- no mem2reg/dataflow change
- no profitability sub-filter widening
- only a direct-home proof at the backend-slot planning layer

### 3. Exact effect on the winner

After the new direct-home proof, `tbool.set_bool` becomes:
...
Measured new selection:

- `set_bool`: **193 generated bytes / 19 insns**, `captured=187/19`

So this item removes **12 instructions / 145 generated bytes** from the
pre-fix current MIR path, and the entire improvement is exactly the deleted
pointer-address spill/reload round trip.

### 4. Re-run of the T424 `+1/-3` experiment

This item does **not** re-admit broad pointer object promotion. It leaves the
T424 profitability sub-filter in place and recovers only the actual win.

Compared against `/home/dave/GitHub/dcc/build/mir-t420-after.tsv` and
`.../mir-t420-after-stack.tsv` (the stable 908/930 checkpoint that T421-T424
all left unchanged):

```sh
PATH=$PWD:/home/dave/GitHub/ntvcm:$PATH \
python3 scripts/mir-migration-census.py \
  --output build/t425-after.tsv \
  --compare /home/dave/GitHub/dcc/build/mir-t420-after.tsv \
  --fail-on-regression

PATH=$PWD:/home/dave/GitHub/ntvcm:$PATH \
python3 scripts/mir-migration-census.py \
  --extra-args=-fstack-check \
  --output build/t425-after-stack.tsv \
  --compare /home/dave/GitHub/dcc/build/mir-t420-after-stack.tsv \
  --fail-on-regression
```

Results:

- ordinary: **909/2026 (44.87%)**
  - **+1** newly MIR-emitted: `tbool.set_bool`
  - **0** no-longer-emitted
- stack-check: **931/2128 (43.75%)**
  - **+1** newly MIR-emitted: `tbool.set_bool`
  - **0** no-longer-emitted

So this is the exact narrower-positive version of T424's failed broadening:
the one real winner is kept, the three losers remain unchanged.

### 5. Focused and milestone validation

Focused full-mode check on the winner and the three prior losers:

```sh
PATH=$PWD:/home/dave/GitHub/ntvcm:$PATH \
pwsh ./scripts/runall.ps1 -Apps tbool,tc99apar,tdecl -Mode full -RunTimeout 20
```

Result: **3/3 passed, 0 regressions**; improvements:

- `tbool` peep cycles **57,356 -> 57,134 (-0.39%)**
- `tbool` peep bytes **8,320 -> 8,192 (-1.54%)**
- `tbool` nopeep cycles **59,951 -> 59,555 (-0.66%)**
- `tbool` nopeep bytes **8,576 -> 8,448 (-1.49%)**
- `tdecl` nopeep cycles **54,171 -> 54,161 (-0.02%)**

The three prior losers (`read_paren_const`, `read_paren_restrict`,
`pick_same_node`) remain correctness-clean and non-regressing under the same
focused run.
...
T424's "safe single-use pointer parameters" were **not** a semantic dead end;
they were a **cost-placement** problem. Full object promotion is too wide for
trivial one-use pointer shapes whose current path is already zero-slot. The
cheap fix is to bypass object promotion entirely and teach the spilled backend
that an objectless, single-use, classifier-safe pointer parameter can keep its
...
## Item T430: block-cse-cost now has a real multi-use address rematerialization planner, but no safe profitable rollout yet (+0, 2026-08-08)

**Starting point (freshly re-derived, not cached):**

- base commit `535f42a`
- ordinary census: **914/2026 (45.11%)**
- stack-check census: **936/2128 (43.98%)**
- `block-cse-cost`: **99 ordinary / 106 stack-check**

I re-ran the exact current `block-cse-cost` population before touching code.
The ordinary list is now:

`a1:op_bit`, `a1:op_pop_pf`, `cint:emit_load_lvalue`,
`cint:emit_store_lvalue`, `cint:init_run_storage`, `cint:mark_get`,
`cint:mark_set`, `cint:popv`, `cint:pushv`, `cint:while_stmt`,
`cobint:compile_stmt`, `tanonagg:check_global_initializers`,
`tanonagg:check_local_initializers`, `tbcreld:main`, `tbitfld:main`,
`tbitfld:make_bits`, `tbitfld:make_signed`, `tbitfld:sum_bits`,
`tbool:check_deref_and_arrays`, `tbool:check_globals`,
`tbool:check_locals`, `tc89bit:tbone`, `tc89bit:tbtwo`,
`tc89comp:cap4`, `tc89comp:cas5`, `tc89comp:slice_sum`,
`tc89decl:tdcl`, `tc89decl:tfpa`, `tc89decl:ttyp`, `tc89fcmp:mkf`,
`tc89ptr:tpv00`, `tc99init:check_local_designators`, `tclit:two_pairs`,
`tcrcfix:crc_t_bits_probe`, `tcrcfix:init_crc_tbl`,
`tcrcfix:test_argv_expr`, `tfarrsub:main`, `tfarrsub:set_direct`,
`tfarrsub:set_intvec`, `tfarrsub:set_pair`, `tfcarg2d:main`,
`tfdf:fdf`, `tfloat4:test_arrays`, `tfloat4:test_constants`,
`tfloat4:test_structs`, `tfmadd:array_case`, `tfmaddr:bits`,
`tfmaddr:fmaddr`, `tfmaf:bits`, `tfmaf:fmadd_`, `tfmaf:fmaf_`,
`tfmaf:is_nan_bits`, `tfmodfsp:frombits`, `tfmodfsp:okbits`,
`tgnarly:print_point`, `tinitreg:tnest`, `tiyreg:main`,
`tlongopt:test_local_long_assign_value`, `tmatbit:main`,
`too:test_machine`, `too:test_scope`, `too:test_shapes`,
`too:test_world`, `tpeepal:main`, `tpfauto:main`, `tpostidx:main`,
`tpostinc:test_char_ptr_math`, `tpostinc:test_int_ptr_math`,
`tpostinc:test_int_simple`, `tpostptr:test_arrays_and_structs`,
`tpostptr:test_requested_forms`, `tpragstk:unguarded_leaf`,
`tptrinit:main`, `tptrlhs:touch_alias_mix`, `tptrlhs:touch_globals`,
`tptrlhs:touch_params`, `treg:test_long_indirect_shift_reg`,
`tscanf:test_sscanf_strings`, `tsretret:main`, `tstr2:test_memchr`,
`tstr2:test_strncpy`, `tstri2:main`, `tstruct:main`,
`tstruct:ret_pair_value`, `tstruct:test2`, `tstructi:main`,
`tstructp:main`, `tstructv:main`, `tstructv:proto_make_big`,
`tstructv:proto_make_pair`, `tstructv:proto_make_wrap`,
`tstructv:proto_sum_big`, `tstructv:proto_sum_two`,
`tstructv:proto_sum_wrap`, `tunbf:main`, `tunion2:main`,
`tunion2:make_upair`, `tvla:vla_ptr2d_deref_chain_compound`,
`wumpus:stats`.

**What I built**

T407's confirmed problem was "same-block address/value reuse becomes
unprofitable because the reused address stops being cheap/rematerializable and
acquires a real retained home". I implemented the missing first slice of that
physical-home model for the spilled selector:

- block-cse retries now re-enable address rematerialization before re-emission;
- rematerialization no longer stops at a single root `MIR_ADDRESS`;
- same-block retries can now rematerialize **multi-use named address chains**
  (`MIR_ADDRESS`, constant-offset `MIR_MEMBER_ADDRESS`, and constant-index
  `MIR_INDEX_ADDRESS`) recursively, so the retry can keep a
  **"no retained home needed"** plan instead of manufacturing a new slot/IY
  home purely because CSE merged repeated address trees.

This is real option-`(b)` architecture: the retry now distinguishes between
"this address must have a physical home" and "this address is still
rematerializable at each use site" instead of inferring cost purely from
address identity.

**What it changed**

With the production gate left unchanged, the new planner changes only rejected
candidate metrics (32 apps in the ordinary census) and leaves the selected
output set unchanged:

- ordinary: **914/2026 -> 914/2026**
- stack-check: **936/2128 -> 936/2128**
- no selected-output regressions

Representative raw candidate improvements:

- `a1:op_pop_pf` generated bytes **1336 -> 1257**, insns **127 -> 120**
- `tcrcfix:init_crc_tbl` improved to **db -150 / di -16**
- `wumpus:stats` improved to **db -10 / di -4** while staying on fallback

**Promotion experiments attempted and reverted**

I then tried the obvious next structural promotion: admit the one-block
`spilled-scalar-cfg` subset whose retry now eliminates **only address
expressions**, stays no-worse on raw bytes/insns, and does not increase spill
/ fixed / operand / phi move counts. That provisional gate admitted:

- `tfarrsub:set_intvec`
- `wumpus:stats`

Fresh census delta from that provisional gate was **+2 ordinary / +2
stack-check**, but focused full-mode validation failed immediately:

- `tfarrsub`: peep **48072 -> 48075** (+0.01%), nopeep
  **50500 -> 50566** (+0.13%)
- `wumpus`: peep **319646 -> 319719** (+0.02%)

Per project rules, I reverted that gate widening completely.

I then forced several additional address-planner candidates individually to see
whether a narrower **structural** cohort exists:

- `a1:op_pop_pf` — **clean win**
- `tcrcfix:init_crc_tbl` — cycles improve, but peep linked size
  **10496 -> 10624** (+1.22%)
- `tc89decl:tfpa` — peep cycles **43788 -> 43830** (+0.1%)
- `tc89fcmp:mkf` — peep **58451 -> 58976** (+0.9%), nopeep
  **59637 -> 60432** (+1.33%)

That is decisive: the new planner is real and useful, but the current
address-only spilled same-block population is **still non-monotonic**. It now
contains at least one clean winner (`a1:op_pop_pf`) and multiple genuine
regressors in the same structural band. There is still no promotable general
predicate here without either:

1. a deeper physical-home model that covers **dynamic-index / loaded-value**
   address bases as first-class rematerializable inputs, or
2. true selector-local rollback/checkpoint semantics so provisional same-block
   retries can be evaluated and discarded transactionally without building a
   production predicate from unstable static proxies.

**Stop condition result**

Land the planner; do **not** land the gate. Final validated coverage delta is
therefore **+0 ordinary / +0 stack-check**. This is an honest negative result,
but it is not a no-op: the work lands a real retained/rematerialized
...
## Item T437: admitted the bounded scalar/acyclic text-size cohort (+149 ordinary/+169 stack-check, 2026-08-08)

Fresh T436 baseline: **1100/2029 ordinary (54.21%)** and **1131/2131
stack-check (53.07%)**. `text-size` was the largest remaining reason at 305
ordinary functions.

### Fresh full-reason result and per-function bisection

Forcing all 305 candidates after T435/T436 failed nine ordinary apps
(`pint`, `tm1mu`, `ts32`, `wumpus`, `tbug`, `tc99varm`, `cint`, `tsvbuf2`,
and `tvla`) plus extended c-testsuite case `00158`. Wrong output ranged from
`tm1mu` losing every result, through signed/narrow shift errors in `ts32`,
to interpreter/I/O timeouts.

Only 22 text-size candidates occur in those nine ordinary apps. A sequential
forced-function fast-mode matrix reduced the individual failures to five:

| function | structural class / observed failure |
| --- | --- |
| `tm1mu.mulmod` | one-block wide return; generic lowering loses specialized `__m1mu` edge-case semantics |
| `ts32.main` | 52 calls, +2,224 generated bytes; narrow/wide shift output wrong |
| `tbug.swfc` | CFG backedge |
| `tsvbuf2.expect_prefix` | CFG backedge |
| `tvla.vla_goto_out` | VLA plus CFG backedge |

Every other candidate in the nine failing apps passed alone. The whole-reason
failures in `pint`, `wumpus`, `tc99varm`, and `cint` were therefore
combination/resource effects, not individually unsafe candidates.

### Structural cohort

Added `wide_values` to candidate-matrix output, complementing T436's semantic
fields. `mir_text_size_coverage_is_semantically_eligible()` requires:

1. at most 64 CFG blocks;
2. at most 32 calls;
3. no VLA;
4. no wide MIR values;
5. no CFG backedge;
6. no inline-substitution call;
7. no declared pointer array;
8. no label-only PHI fallthrough;
9. generated text no more than 2,048 bytes over captured legacy text.

This deliberately reuses T436's CP/M memory-growth boundary and excludes all
five individually unsafe shapes by their architecture, never by app/function
name.

The complete resulting cohort passed together:

```text
pwsh ./scripts/runall.ps1 -Mode full -Extended -NoPerfCheck
314/314 runnable apps passed, 0 failed
extended/diagnostics/dccpeep passed
```

### Result and validation

- Ordinary: **1100/2029 -> 1249/2050 (60.93%)**, **+149 MIR functions**,
  zero removals.
- Stack-check: **1131/2131 -> 1300/2156 (60.30%)**, **+169 MIR
  functions**, zero removals.
- The denominator grows by 21 ordinary/25 stack-check rows because accepted
  callers mark previously dormant deferred static bodies as needed; those
  functions now genuinely materialize and enter the census.
- Remaining `text-size`: **196 ordinary / 213 stack-check**.
- Forced-MIR regression harness: PASS (9/9).
- Performance-enabled full extended gate: correctness clean, with 235
  deliberate Phase-1 regressions and 11 improvements.
- `-UpdatePerfBaseline`: **314/314 passed**, measured profile recorded.

The size/cycle regressions are large by design in this coverage-first phase;
they are now explicit Phase-2 input rather than hidden selection gates.

### Next

The bounded scalar/acyclic text-size vein is exhausted. Re-rank the remaining
...
## Item T440: terminal bounded acyclic dynamic-index-base cohort (+13 ordinary/+13 stack-check, 2026-08-08)

Fresh T439 baseline: **1403/2051 ordinary (68.41%)** and **1464/2164
stack-check (67.65%)**. `dynamic-index-base-cost` was the next-largest
ordinary reason at 93 functions.

### Full reason and individual failures

Forcing the complete reason failed nine ordinary apps plus extended case
`00158`. A sequential forced-function matrix over the 24 candidates in those
apps isolated five individually unsafe functions:

| function | structural class |
| --- | --- |
| `tallocx.t_stress` | 32 blocks, 26 calls, wide backedge, label-only PHI |
| `tmodp2.main` | wide backedge, +8,380 bytes |
| `ttrig.main` | wide backedge |
| `tforsco.main` | 69 blocks, 33 calls, wide backedge, +4,747 bytes |
| `adaint.add_var_name` | label-only PHI fallthrough |

All other candidates in the failing apps passed alone. The clean structural
boundary is therefore acyclic, no label-only PHI, at most 64 blocks/32 calls,
no VLA/inline-substitution/pointer-array, and at most 2 KiB growth.

### Combination-only pint resource boundary

Applying that rule at the actual final decision point proposed 14 functions.
The complete cohort failed only `pint`'s nopeep `ttt.pas` scenario. Its three
new candidates (`add_sym`, `add_var_name`, `for_stmt`) each passed alone and
every pair passed; only all three together failed. `for_stmt` is the sole
cohort member over 5,000 generated bytes (5,699). Adding a 5,000-byte
absolute candidate ceiling excludes only that function and provides a
defensive CP/M linked-image/resource bound alongside the relative 2 KiB cap.

The resulting 13-function cohort passed full extended correctness.

### Production rule and result

`mir_dynamic_index_base_coverage_is_semantically_eligible()` runs at the true
final decision point and requires:

- at most 64 blocks and 32 calls;
- no VLA, CFG backedge, inline-substitution call, pointer array, or label-only
  PHI fallthrough;
- at most 5,000 generated bytes;
- at most 2,048 bytes growth over captured legacy text.

- Ordinary: **1403/2051 -> 1416/2051 (69.04%)**, **+13**, zero removals.
- Stack-check: **1464/2164 -> 1477/2164 (68.25%)**, **+13**, zero removals.
- Remaining `dynamic-index-base-cost`: **80 ordinary / 81 stack-check**.
- Full extended correctness: **314/314 runnable apps**, diagnostics,
  dccpeep, and extended corpus pass.
- Forced-MIR regression harness: PASS (9/9).
...
work is backedge/wide, label-PHI, oversized/resource-sensitive, or is
reclassified by a later retry. Re-rank against `boolean-phi-cost` (97),
remaining `text-size` (69), and `unary-not-cost` (61).

## Item T441: terminal bounded acyclic unary-not cohort (+15 ordinary/+15 stack-check, 2026-08-08)

Fresh T440 baseline: **1416/2051 ordinary (69.04%)** and **1477/2164
stack-check (68.25%)**. `unary-not-cost` contained 61 ordinary functions.

Forcing the complete reason failed eight apps (`adaint`, `cint`, `pint`,
`fint`, `a1`, `tchess`, `cobint`, and `forint`). A sequential
forced-function matrix covered all 32 candidates in those apps. Only one
failed individually:

```text
tchess.find_legal_text_move
```

It has a CFG backedge (6 blocks, 4 calls). Every acyclic candidate in the
failure set passed alone.

Renamed T440's exact formula to the shared
`mir_bounded_acyclic_coverage_is_semantically_eligible()` and reused it at
the true final decision point for `unary-not-cost`. It requires at most 64
blocks/32 calls, no VLA/backedge/inline-substitution/pointer-array/label-PHI,
at most 5,000 generated bytes, and at most 2,048 bytes growth.

The resulting cohort passed full extended correctness:

- Ordinary: **1416/2051 -> 1431/2052 (69.74%)**, **+15**, zero removals.
- Stack-check: **1477/2164 -> 1492/2165 (68.91%)**, **+15**, zero removals.
- Remaining `unary-not-cost`: **47 ordinary / 48 stack-check**.
- Full extended correctness: **314/314 runnable apps**, diagnostics,
  dccpeep, and extended corpus pass.
- Forced-MIR regression harness: PASS (9/9).
- Performance gate: 29 deliberate Phase-1 regressions, 6 improvements;
  `-UpdatePerfBaseline` completed with **314/314 passed**.
...
## Item T443: terminal call-containing bounded wide-store cohort (+14 ordinary/+14 stack-check, 2026-08-08)

Fresh T442 baseline: **1466/2052 ordinary (71.44%)** and **1527/2165
stack-check (70.53%)**. `wide-store-cost` contained 38 ordinary functions.

Forcing the complete reason failed `pint` and `ttrig`. Individual full-mode
tests over their six candidates found:

- unsafe: `pint.calc_code_limit`, `ttrig.logf`, `ttrig.xsinf`;
- safe alone: `ttrig.cosf`, `ttrig.powf`, `ttrig.tanf`.

The trig failures are backedge and/or over 5,000 generated bytes, already
excluded by the shared bounded-acyclic predicate. The resulting 15-function
terminal cohort failed only `pint`.

`calc_code_limit` is a 3-block, call-free function with one local `long`.
Forced MIR fails with the normal stack-check build but passes both modes with
`-NoStackCheck`, proving a peak-stack/resource boundary rather than arithmetic
miscompilation. Its MIR frame is 12 bytes (4 local + four backend units),
enough to cross pint's already-tight full-I/O workload limit.

It is the 15-function cohort's only call-free member. A reason-specific
wrapper, `mir_wide_store_coverage_is_semantically_eligible()`, requires
`mir_call_count() > 0` in addition to the shared bounded-acyclic predicate.
The resulting 14 functions pass together.

- Ordinary: **1466/2052 -> 1480/2052 (72.12%)**, **+14**, zero removals.
- Stack-check: **1527/2165 -> 1541/2165 (71.18%)**, **+14**, zero removals.
- Remaining `wide-store-cost`: **24 ordinary / 26 stack-check**.
- Full extended correctness: **314/314 runnable apps**, diagnostics,
  dccpeep, and extended corpus pass.
- Forced-MIR regression harness: PASS (9/9).
- Performance gate: 37 deliberate Phase-1 regressions, 3 improvements;
  `-UpdatePerfBaseline` completed with **314/314 passed**.
...
## Item T448: concrete PHI source slots unlock boolean and binary-load loops (+28 ordinary/+34 stack-check, 2026-08-08)

Fresh T447 baseline: **1541/2052 ordinary (75.10%)** and **1615/2165
stack-check (74.60%)**.

### Root cause: a PHI source was planned and emitted as slotless

The cross-reason tiny-loop experiment exposed
`tbcgcol.global_bc_across_byte_loop`. Its backedge has two PHIs (`sum` word,
`count` byte). The decremented `count` value feeds both the immediate branch
and the next-iteration PHI.

Backend planning considered that computed value branch-forwardable and gave
it no slot. The PHI edge copy still called `mir_virtual_load(value)`;
`mir_virtual_offset()` then fell back to treating the SSA value ID as a slot
number, producing an impossible `ix-33` access in a 7-byte frame. Allocating a
slot alone was insufficient because emission still forwarded directly into
the branch and never populated it.

Added shared `mir_value_requires_phi_slot()` for any PHI source that cannot be
rematerialized. Both backend planning and `mir_emit_virtual_store()` use it:

- planning bypasses direct-home, stack, cache, and branch-only slot elision;
- emission forces a real store before any immediate branch handoff.

The invalid offset disappears and the tbcgcol loop passes both modes.

### Coverage cohorts after the fix

Re-running full `binary-load-pair-cost` forcing now fails only the known
single-call `pint.emit` stack/resource case. The terminal binary-load policy
therefore widens from bounded acyclic only to every `mir_call_count() != 1`
shape, adding four loop functions.

Full boolean-PHI forcing drops from 12 failing apps to 8, fixing
`tlngnarw`, `wumpus`, `forint`, and `tc99varm`. A post-fix small-loop cohort
requires:

- a backedge, at most 20 blocks/32 calls;
- no VLA, inline-substitution call, pointer array, or label-only PHI;
- at most 2,500 generated bytes and 2,048 bytes growth.

The 2,500-byte cap excludes `pint.skip_brace_comment`; leave-one-out testing
proved that candidate was the sole interaction trigger among pint's remaining
small loops. The final boolean cohort adds 24 ordinary functions.

- Ordinary: **1541/2052 -> 1569/2052 (76.46%)**, **+28**, zero removals.
- Stack-check: **1615/2165 -> 1649/2165 (76.17%)**, **+34**, zero removals.
- Remaining `boolean-phi-cost`: **81 ordinary / 86 stack-check**.
- Remaining `binary-load-pair-cost`: **2 ordinary / 2 stack-check**.
- Full extended correctness: **314/314 runnable apps**, diagnostics,
  dccpeep, and extended corpus pass.
- Forced-MIR regression harness: PASS (9/9).
...
## Item T450: nested inline temp scope, oversized text tail, and post-PHI block CSE (+33 ordinary/+37 stack-check, 2026-08-08)

Fresh T449 baseline: **1634/2060 ordinary (79.32%)** and **1730/2179
stack-check (79.39%)**.

### Nested inline temp lifetimes

T447's `inline-temp-overlap` gate exposed the root cause in
`tinline.inline_temp_collision_check`. `mir_inline_live_temp_mask` and
`mir_inline_expand_depth` remained active only while cloning the inline AST;
they were restored before the cloned body was lowered. Nested inline calls
therefore saw outer `#itmp1` as free and overwrote it before the outer load.

`mir_try_lower_inline_call_common()` now leaves mask/depth scope active for
the caller to lower the cloned expression/statement, then restores it through
`mir_end_inline_call_scope()`. The nested argument moves to `#itmp2`, the
collision test returns 5844 again, and one function becomes MIR-emitted.

### Final text-size tail

After T449 only four oversized functions remained. Full reason forcing proved
`tatof.main`, `tmathf.main`, and `tzpad.main` correct; all have at least 80
calls. `ts32.main` (52 calls) still miscompiles narrow signed/unsigned shifts.
The terminal text predicate now accepts either <=10,000 bytes or >=80 calls,
admitting the three proven functions while retaining ts32.

### Post-PHI block CSE

Re-running full block-CSE forcing after T448/T449 leaves only two failures:

- `tpfauto.main`: the unique wide, exactly-20-call semantic failure;
- `tstructv.main`: 26,218 generated bytes and CP/M memory exhaustion.

Terminal block-CSE acceptance requires <=10,000 bytes and excludes the
wide/20-call stratum. It admits 29 functions together; five oversized
functions plus tpfauto remain.

- Ordinary: **1634/2060 -> 1667/2060 (80.92%)**, **+33**, zero removals.
- Stack-check: **1730/2179 -> 1767/2179 (81.09%)**, **+37**, zero removals.
- Remaining `text-size`: **1 ordinary / 1 stack-check** (`ts32.main`).
- Remaining `block-cse-cost`: **6 ordinary / 6 stack-check**.
- Full extended correctness: **314/314 runnable apps**, diagnostics,
  dccpeep, and extended corpus pass.
- Forced-MIR regression harness: PASS (9/9).
- Performance gate: 83 deliberate Phase-1 regressions, 1 improvement;
  `-UpdatePerfBaseline` completed with **314/314 passed**.

## Item T451: non-speculative scalar dynamic-index loops (+44 ordinary/+44 stack-check, 2026-08-08)

Fresh T450 baseline: **1667/2060 ordinary (80.92%)** and **1767/2179
stack-check (81.09%)**.

The remaining dynamic-index-base population was still the second-largest
reason. T440 already admitted its bounded acyclic stratum. The five
individually known failures all lie outside this new loop boundary (wide,
label-PHI, oversized, or >64 blocks/>32 calls).

The T451 cohort requires:

- a CFG backedge;
- at most 64 blocks and 32 calls;
- scalar-only (no wide values);
- no VLA, inline-substitution call, pointer array, or label-only PHI;
- at most 5,000 generated bytes and 2,048 bytes growth.

Wildcard testing proposed 45 functions but failed only pint. The visible
final functions (`find_proc`, `vars`) passed alone and together.
Instrumentation found hidden speculative matches (`find_scope`, `call_proc`).
Requiring `!g_speculative_codegen_active` preserves search behavior and
retains 44 final functions; the complete cohort then passes.

- Ordinary: **1667/2060 -> 1711/2060 (83.06%)**, **+44**, zero removals.
- Stack-check: **1767/2179 -> 1811/2179 (83.11%)**, **+44**, zero removals.
- Remaining `dynamic-index-base-cost`: **36 ordinary / 37 stack-check**.
- Full extended correctness: **314/314 runnable apps**, diagnostics,
  dccpeep, and extended corpus pass.
- Forced-MIR regression harness: PASS (9/9).
...
## Item T453: bounded-wide dynamic indexes and larger acyclic wide stores (+11 ordinary/+11 stack-check, 2026-08-08)

Fresh T452 baseline: **1720/2060 ordinary (83.50%)** and **1822/2179
stack-check (83.62%)**.

Two reason-specific wide strata were validated together:

1. `dynamic-index-base-cost`: <=64 blocks/32 calls, wide values, no
   VLA/inline/pointer/label-PHI, <=10,000 generated bytes, non-speculative.
   This excludes every known unsafe wide driver (`ttrig`, `tmodp2`,
   `tforsco`) and adds seven.
2. `wide-store-cost`: acyclic, call-containing, no
   VLA/inline/pointer/label-PHI, <=10,000 bytes. This extends T443 above
   5 KiB, adding four safe functions while retaining call-free
   `pint.calc_code_limit`, backedge `ttrig.xsinf`, and oversized failures.

- Ordinary: **1720/2060 -> 1731/2060 (84.03%)**, **+11**, zero removals.
- Stack-check: **1822/2179 -> 1833/2179 (84.12%)**, **+11**, zero removals.
- Remaining `dynamic-index-base-cost`: **29 ordinary / 30 stack-check**.
- Remaining `wide-store-cost`: **20 ordinary / 22 stack-check**.
- Full extended correctness: **314/314 runnable apps + 196/196 extended**,
  diagnostics and dccpeep pass.
- Forced-MIR regression harness: PASS (9/9).
- Performance gate: 33 deliberate Phase-1 regressions;
  `-UpdatePerfBaseline` completed with **314/314 passed**.

## Item T454: VLA dynamic indexes and complete terminal wide constants (+20 ordinary/+22 stack-check, 2026-08-08)

Fresh T453 baseline: **1731/2060 ordinary (84.03%)** and **1833/2179
stack-check (84.12%)**.

Validated two cohorts together:

1. Five VLA `dynamic-index-base-cost` functions with <=16 blocks/5 calls,
   no inline/pointer/label-PHI shape, and <=5,000 generated bytes. Full
   reason forcing no longer had VLA app failures after the PHI/pointer fixes.
2. Every remaining non-speculative terminal `wide-constant-cost` function.
   The historical blind-force `tpfauto.main` failure is transient; terminal
   main is still `block-cse-cost`, so it is not intercepted.

- Ordinary: **1731/2060 -> 1751/2060 (85.00%)**, **+20**, zero removals.
- Stack-check: **1833/2179 -> 1855/2179 (85.13%)**, **+22**, zero removals.
- Remaining `dynamic-index-base-cost`: **24 ordinary / 25 stack-check**.
- Remaining terminal `wide-constant-cost`: **0**.
- Full extended correctness: **314/314 runnable apps + 196/196 extended**,
  diagnostics and dccpeep pass.
- Forced-MIR regression harness: PASS (9/9).
