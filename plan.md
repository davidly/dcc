...
- Published HEAD before this batch: `523c251`
  (`schedule float truth combinators`).
- Current candidate coverage: **1917/2185 (87.73%)**.
- Remaining fallback population: **268 `final-cost-policy`**, with no other
  fallback reason.
...
- The other statically smaller candidates are also false wins:
  ...
  (+28.46%/+31.34%).
- Current next priority: repeated causes in the 268 final-cost fallbacks,
