...
static int nz_long(const struct Bag *b, int i)
{
    if (!b->lv[i])
        return 0;
    return 1;
...
static int nz_float(const struct Bag *b, int i)
{
    if (!b->fv[i])
        return 0;
    return 1;
...
static int nz_ptr(const struct Bag *b, int i)
{
    if (!b->pv[i])
        return 0;
    return 1;
...
int main(void)
{
    ...
    printf("nested for primes=%d gap=%d p79=%d\n",
           count_primes(&s), largest_gap(&s), !s.composite[79]);
    printf("nested index l=%04d f=%04d p=%04d gp=%04d row=%03d\n",
           lmask, fmask, pmask, gmask, rmask);
    printf("nested stride sum=%d down=%d\n",
