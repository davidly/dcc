...
static int sliding_wave7(const int *a, int n, int k, int *out)
{
    int dq[CAPACITY], head = 0, tail = 0, count = 0, i;
    for (i = 0; i < n; ++i) {
        int limit = i - k + 1;
...
static int reference_maximum(
    const int input[], int length, int window, int output[])
{
    int count = 0;
    int first;
...
static int run_case(const int input[], int length, int window)
{
    int actual[CAPACITY + 4];
...
int main(void)
{
    static const int descending[CAPACITY] =
        {9, 8, 7, 6, 5, 4, 3, 2};
    static const int duplicates[CAPACITY] =
        {-7, -7, -9, -7, -8, -8, -6, -6};
    static const int alternating[CAPACITY] =
