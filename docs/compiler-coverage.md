...
Forty-one `ln2` field mutations cover every named log-series rejection group
and each array-identity/local-location condition while executing the original
program generically; five source-level reorderings provide independent
controls. Both mutator functions have exact line, branch, and region coverage.
...
The full clobber manifest contains 992 target configurations.
...
| Native branch outcomes | 89,935 / 144,510 | 62.23% |
...
matrix covers another 27 branch outcomes, and the per-condition log array/local
matrix covers another 29. The raw ledger is now 54,328.

- Review the remaining 54,328 unreviewed raw uncovered outcomes rather than
